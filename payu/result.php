...
if($ipn->validateReceived()){
	...
	if($orderid > 0){
		...
		$query = 'UPDATE '._DB_PREFIX_.'order_history SET id_order_state = '.(int)$orderNewState.' WHERE id_order = '.(int)$orderid.' limit 1';
		Db::getInstance()->execute($query);
		
		
		$query2 = "UPDATE "._DB_PREFIX_."orders SET invoice_date = '".$invDate."' WHERE id_order = ".(int)$orderid." limit 1";
